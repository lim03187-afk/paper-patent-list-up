...
export const INITIAL_DOCUMENTS: ResearchDocument[] = [
  {
    ...
    year: "2017",
    summary: "[제목: Attention Is All You Need | 저자: Ashish Vaswani et al. | 발행연도: 2017년] 기존의 순환 신경망(RNN)과 합성곱 신경망(CNN) 구조를 완전히 배제하고 오직 어텐션 메커니즘(Attention Mechanism)만으로 구성된 Transformer 아키텍처를 최초로 제안한 논문. 자연어 처리 및 현대 생성형 AI의 기반이 됨.",
  ...
  },
  {
    ...
    year: "2018",
    summary: "[제목: Method and apparatus for neural network-based machine translation | 저자: Google LLC | 발행연도: 2018년] 심층 신경망을 활용한 다국어 기계 번역 시스템 및 인코더-디코더 아키텍처 구현에 관한 핵심 특허. 문맥 기반 어텐션 가중치 계산 및 실시간 추론 최적화 방법론 포함.",
  ...
  },
  {
    ...
    year: "2021",
    summary: "[제목: LoRA: Low-Rank Adaptation of Large Language Models | 저자: Edward J. Hu et al. | 발행연도: 2021년] 거대 언어 모델의 전체 파라미터 미세조정 대신 저랭크(Low-Rank) 행렬 분해 방식을 도입하여, 학습 가능한 파라미터 수를 극적으로 줄이면서도 성능을 유지하는 고효율 파인튜닝 기법.",
  ...
  },
  {
    ...
    year: "2022",
    summary: "[제목: High-Resolution Image Synthesis with Latent Diffusion Models | 저자: Robin Rombach et al. | 발행연도: 2022년] 픽셀 공간이 아닌 압축된 잠재 공간(Latent Space)에서 확산 모델을 작동시켜 계산 비용을 대폭 절감하면서도 고품질 이미지 생성 능력을 입증한 Stable Diffusion의 기반 논문.",
  ...
  },
  {
    ...
    year: "2023",
    summary: "[제목: System and Method for Efficient Prompt Processing in Generative Models | 저자: OpenAI, L.L.C. | 발행연도: 2023년] 생성형 AI 모델에서 사용자 프롬프트 토큰의 캐싱 및 컨텍스트 윈도우 처리 속도를 가속화하기 위한 하이브리드 메모리 관리 특허 기술.",
  ...
  },
  {
    ...
    year: "2022",
    summary: "[제목: FlashAttention: Fast and Memory-Efficient Exact Attention with IO-Awareness | 저자: Tri Dao et al. | 발행연도: 2022년] GPU 하드웨어의 메모리 계층 구조(SRAM과 HBM)를 고려한 I/O 인식 타일링(Tiling) 기법을 통해 트랜스포머의 어텐션 연산 속도를 2~4배 가속화하고 메모리 사용량을 O(N^2)에서 O(N)으로 개선한 논문.",
